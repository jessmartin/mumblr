...
// If user is _not_ logged in, hide the interface.
//   Currently, it starts out hidden. Nothing to do here.

// If user is logged in, show the log in button.
//   Currently, it starts out shown. Nothing to do here.
...
const logIn = document.querySelector<HTMLInputElement>("#log-in")!;
logIn.addEventListener("click", async function () {
  // Initialize WebNative (which will force a log-in)
  // TODO: Why does this not finish before the items below are hidden?
  await main();
  ...
  const bodyInput = document.querySelector<HTMLInputElement>(".body-input")!;
  bodyInput.style.display = "block";
  const postButton = document.querySelector<HTMLInputElement>(".post-button")!;
